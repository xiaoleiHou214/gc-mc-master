...
from gcmc.preprocessing import create_trainvaltest_split, \
    ...
    load_data_monti, load_official_trainvaltest_split, normalize_features, load_wsdream_official_trainvaltest_split
from gcmc.model import RecommenderGAE, RecommenderSideInfoGAE
from gcmc.utils import construct_feed_dict
from gcmc.metrics import HitRate, Diversity
...
ap.add_argument("-d", "--dataset", type=str, default="ml_1m",
                choices=['ml_100k', 'ml_1m', 'ml_10m', 'douban', 'yahoo_music', 'flixster', 'wsdream'],
                help="Dataset string.")
...
ap.add_argument("-hi", "--hidden", type=int, nargs=2, default=[200, 75],
...
if DATASET == 'flixster' or DATASET == 'douban' or DATASET == 'yahoo_music':
    ...
    print("Using wsdream dataset split train.txt/test.txt with 20% validation set size...")
    u_features, v_features, adj_train, train_labels, train_u_indices, train_v_indices, \
        val_labels, val_u_indices, val_v_indices, test_labels, \
        test_u_indices, test_v_indices, class_values = load_wsdream_official_trainvaltest_split(DATASET, TESTING)
else:
...
for i in range(NUMCLASSES):
    ...
    print('support_unnormalized.nnz:',support_unnormalized.nnz)
...
if TESTING:
    test_avg_loss, test_rmse, outputs, lables = sess.run([model.loss, model.rmse, model.outputs, model.labels], feed_dict=test_feed_dict)
    topK_hitRate = HitRate(outputs, lables, test_u_indices)
    topK_Diversity = Diversity(outputs, lables, test_u_indices)
    print('test loss = ', test_avg_loss)
    print('test rmse = ', test_rmse)
    print('topK_hitRate = ', topK_hitRate)
    print('topK_Diversity = ', topK_Diversity)
